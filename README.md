...
- 날짜별 할 일 CRUD, 체크 한 번에 빈칸 → 완료 → 실패로 도는 상태.
- 시작 시각은 **10분 단위**로 붙입니다. 안 붙이면 '종일'이고, 목록은 종일 → 시각순으로 섭니다.
  시각이 순서를 정하므로 드래그 정렬은 종일 항목끼리만 씁니다.
- 반복 루틴은 **크론 없이** 해당 날짜를 처음 열어 볼 때 그날의 할 일로 만들어집니다(기준일 KST, 같은
  루틴이 두 번 생기는 것은 부분 유니크로 차단). 루틴은 종일이라 언제나 목록 맨 위입니다.
- 주별·월별 리포트: 완료율(실패·빈칸 모두 분모)·streak·루틴별 달성률.
