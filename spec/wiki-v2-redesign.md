...
> **⛔ 폐기됨 (removed, 2026-07-19).** 이 재설계로 구현했던 위키 v2를 포함해 Phase 7(LLM 위키·챗봇·RAG 인덱싱) 전체를 코드베이스에서 제거했다 — 사용자 결정(파일 공유 코어 집중, git 이력 보존). 이 문서는 설계 기록으로만 남긴다. 실제 구현·스키마·의존성은 존재하지 않으며, 재도입이 필요하면 관련 커밋 이력을 참조한다.
