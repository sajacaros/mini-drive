...
### 3.5 델타 동기화 — **범위 제외** (2026-07-19 결정)

웹 브라우저 중심 서비스에서는 로컬 파일과의 지속 동기화 클라이언트가 없어 블록 단위 델타 동기화의 효용이 낮다고 판단, **범위에서 제외**한다. 대용량 업로드의 중단/재개 요구는 S3 Multipart 기반 재개 가능 업로드(3.2)가 담당한다. 아래 표는 제외된 원래 범위의 기록이다.

| 기능 (제외됨) | 설명 | 우선순위 |
|---|---|---|
| ~~블록 단위 저장~~ | 대용량 파일을 청크 분할 저장, 해시 기반 중복 제거 | — |
| ~~수정 블록만 동기화~~ | 델타 동기화 전략 (블록 해시 비교) | — |
| ~~롱폴링 알림~~ | 파일 변경 실시간 알림 (WebSocket 또는 Long-Polling) | — |
...
각 페이즈 완료 시 검토·컨펌 후 다음 페이즈로 진행한다.
